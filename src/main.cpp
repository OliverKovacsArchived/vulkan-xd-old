...
private:
    ...
    std::vector<std::vector<float>> ssbos;
    std::vector<std::vector<float>> ubos;
    ...
    void createDescriptorSetLayout() {
        ...
        std::array<VkDescriptorSetLayoutBinding, 4> bindings = {uboLayoutBinding, samplerLayoutBinding, ssboLayoutBindings[0], ssboLayoutBindings[1]};
    ...
    void createModel() {
        ...
        ssbos.resize(storageBufferCount);
        for (size_t i = 0; i < ssbos.size(); i++) {
            ssbos[i].reserve(SSBO_RESERVE_SIZE[i]);
        }
        ...
        idxs.resize(2);
        xdvk::hypercubeIndices(idxs[0], DIMENSION, 0, 0);
        xdvk::icositetrachoronIndices(idxs[1], 0, 0);

        vertices.resize(idxs[1].size());
        ...
        for (size_t i = 0; i < scene.entities.size(); i++) {
            ...
            createIndexBuffer(idxs[i], scene.entities[i].geometry.indexBuffer, scene.entities[i].geometry.indexBufferMemory);
            scene.entities[i].geometry.indexBufferSize = idxs[i].size();
        }
        ...
        ssbos[1].resize(scene.entities.size() * xdvk::transformSize(DIMENSION));

        scene.entities[0].transform.position[0] =  0.5;
        scene.entities[1].transform.position[0] = -0.5;
        ...
        for (size_t i = 0; i < scene.entities.size(); i++) {
            ...
            ssbos[0].resize(index + size);
            std::copy_n(entity.geometry.vertices.data(), size, &ssbos[0][index]);
            index += size;

            std::copy_n(scene.entities[i].transform.buffer, size, &ssbos[1][i * xdvk::transformSize(DIMENSION)]);
    ...
    void createStorageBuffers() {
        ...
        for (size_t i = 0; i < storageBuffers.size(); i++) {
            VkDeviceSize bufferSize = ssbos[i].size() * sizeof(ssbos[i][0]);
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            ...
            // shader storage
            std::array<VkDescriptorBufferInfo, 2> storageBufferInfo{};
            storageBufferInfo[0].buffer = storageBuffers[0][i];
            storageBufferInfo[0].offset = 0;
            storageBufferInfo[0].range = ssbos[0].size() * sizeof(ssbos[0][0]);

            storageBufferInfo[1].buffer = storageBuffers[1][i];
            storageBufferInfo[1].offset = 0;
            storageBufferInfo[1].range = ssbos[1].size() * sizeof(ssbos[1][0]);
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        // glm::vec3 scale = glm::vec3(400.0f, 400.0f, 400.0f);
        // glm::vec3 scale = glm::vec3(1.0f, 1.0f, 1.0f);

        UniformBufferObject ubo{};
        // ubo.model = glm::scale(glm::rotate(glm::mat4(1.0f), time * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f)), scale); // original
        // ubo.model = glm::scale(glm::rotate(glm::mat4(1.0f), time * glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f)), scale);
        // ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f)); // original
        // ubo.view = glm::lookAt(glm::vec3(0.0f, -0.1f, 5.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    void updateStorageBuffer(uint32_t currentImage) {
        ...
        size_t i = 0;
        for (auto &entity : scene.entities) {
            for (size_t j = 0; j < DIMENSION * (DIMENSION - 1) / 2; j++) {
                entity.transform.rotation[j] = time * (i + 1);
            }
            i++;
        }
        ...
        for (size_t i = 0; i < storageBuffersMemory.size(); i++) {
            void* data;
            vkMapMemory(device, storageBuffersMemory[i][currentImage], 0, ssbos[i].size() * sizeof(ssbos[i][0]), 0, &data);
                memcpy(data, ssbos[i].data(), ssbos[i].size() * sizeof(ssbos[i][0]));
