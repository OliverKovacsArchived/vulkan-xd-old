...
void fetchTransform(inout Transform transform, int transform_index, int stride, int offset) {
    ...
    // int index = offset + transform_index * t_n;
...
void main() {
    ...
    for (int i = 0; i < n; i++) {
        transform.rotation[i] = ubo.time * 0.1 * (i + 0) * (constants.entity + 1.0);
    }
    ...
    float rgb_s = 1.0;
    vec3 color = vec3(
        sin(gl_VertexIndex * rgb_s + time + 0 * pi / 3),
        sin(gl_VertexIndex * rgb_s + time + 2 * pi / 3),
        sin(gl_VertexIndex * rgb_s + time + 4 * pi / 3)
    );
    fragColor = color;
